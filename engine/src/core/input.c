...
void input_process_key(keys key, b8 pressed) {
    //only handle this if state acutally changed
...
b8 input_is_button_down(buttons button) {
    ...
    return state.mouse_current.buttons[button] == TRUE; //by default all buttons are up state if we arent initialized down state should
